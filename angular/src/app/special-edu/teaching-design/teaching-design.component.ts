...
@Component({
  ...
  imports: [CommonModule, FormsModule, NzCardModule, NzFormModule, NzInputModule, NzInputNumberModule, NzSelectModule, NzButtonModule, NzSpinModule, NzTagModule, NzDividerModule, NzTableModule, NzGridModule, NzTabsModule, NzModalModule, NzRadioModule],
  template: `
    ...
    <nz-tabset [(nzSelectedIndex)]="activeTab">
          ...
          <div nz-col [nzSpan]="15">
            <nz-spin [nzSpinning]="generating()">
              @if (result(); as r) {
                ...
                <p style="color:#999">左侧填写后点击生成，生成后自动保存为草稿，可在“方案列表”中提交审核。</p>
              }
            </nz-spin>
          </div>
        </div>
      ...
      <nz-tab [nzTitle]="'待审核 (' + pending().length + ')'">
        <nz-table [nzData]="pending()" nzSize="small" style="margin-top:12px">
          <thead><tr><th>标题</th><th>类别</th><th>提交人</th><th>指派审核教师</th><th>操作</th></tr></thead>
          <tbody>
            @for (h of pending(); track h.id) {
              <tr><td>{{ h.title }}</td><td>{{ h.categoryName }}</td><td>{{ h.creatorId }}</td>
  ...
  <ng-template #extraTpl><span style="color:#888">生成即存草稿 · 审核后发布</span></ng-template>
