...
export class IndexingJobService {
  ...
  getList = (input: GetIndexingJobsInput, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<IndexingJobDto>>({
      method: 'GET',
      url: '/api/app/indexing-job',
      params: { resourceId: input.resourceId, status: input.status, startTime: input.startTime, endTime: input.endTime, filter: input.filter, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
