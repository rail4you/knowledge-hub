...
public class ResourceRecommendationAppService : KnowledgeHubAppService, IResourceRecommendationAppService
{
    ...
    public async Task<List<RecommendedResourceDto>> GetRelatedResourcesAsync(Guid resourceId, int count = 6)
    {
        ...
        {
            var reason = "相关资源";
            var coviewRaw = reader.IsDBNull(reader.GetOrdinal("Score")) ? 0 : reader.GetDouble(reader.GetOrdinal("Score"));
            if (reader.GetString(reader.GetOrdinal("CategoryName")) != null &&
    ...
    {
        var ordinal = reader.GetOrdinal("Score");
        return new RecommendedResourceDto
        {
            ...
            AverageRating = Math.Round(reader.GetDouble(reader.GetOrdinal("AvgRating")), 1),
            TotalReviews = reader.GetInt32(reader.GetOrdinal("TotalReviews")),
            RecommendationScore = Math.Round(reader.GetDouble(ordinal), 4),
