...
async def md_to_tree(md_path, if_thinning=False, min_token_threshold=None, if_add_node_summary='no', summary_token_threshold=None, model=None, if_add_doc_description='no', if_add_node_text='no', if_add_node_id='yes'):
    ...
    # If no headings found, use LLM to generate structure
    if not node_list and model:
        print(f"No Markdown headings found. Using LLM to generate document structure...")
        llm_entries = await generate_structure_from_unstructured_md(markdown_content, model=model)
        if llm_entries:
            print(f"LLM identified {len(llm_entries)} sections. Injecting headings into Markdown...")
            # Inject headings into markdown_lines at the specified positions
            # Process in reverse order so line_num offsets remain valid
            for entry in sorted(llm_entries, key=lambda e: e['line_num'], reverse=True):
                heading_prefix = '#' * entry['level']
                heading_line = f"{heading_prefix} {entry['title']}"
                idx = entry['line_num'] - 1  # Convert to 0-based index
                markdown_lines.insert(idx, heading_line)

            # Update line_count and re-extract
            markdown_content = '\n'.join(markdown_lines)
            line_count = markdown_content.count('\n') + 1
            node_list, markdown_lines = extract_nodes_from_markdown(markdown_content)
            print(f"After LLM heading injection: {len(node_list)} nodes found.")
