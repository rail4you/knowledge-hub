...
![文档搜索结果示例](screenshots/admin-search-results.png)

> **上图说明:** 搜索关键词"教育"后返回的结果。每条结果卡片包含:资源名称、匹配位置（"第 1 页"）、相关度分数（87分、85分、84分）、文件类型标签（.pdf/.docx）、评价入口（星星图标）以及内容预览（关键词"教育"被高亮显示）。

**文档类型与视频类型的搜索结果差异:**

系统根据资源类型（文档/视频）采用不同的解析和展示方式:

| 对比项 | 文档搜索结果 | 视频搜索结果 |
|--------|-------------|-------------|
| **解析单位** | 按页解析 | 按时间轴解析 |
| **位置标签** | 显示"第 X 页" | 显示时间范围,如"00:01:30 - 00:03:45"（紫色标签） |
| **文件类型图标** | file-pdf、file-word 等文档图标 | video-camera 视频图标 |
| **文件类型标签** | .pdf、.docx、.pptx 等 | "视频" |
| **点击行为** | 跳转到文档查看器,自动定位到匹配页码 | 弹出视频播放器,自动跳转到匹配时间点 |
| **预览内容** | 包含关键词的文档段落上下文 | 时间点附近的事件描述 |
| **评价入口** | 卡片右侧星星图标 | 卡片右侧星星图标 |
...
┌──────────────────────────────────────────────────────────────┐
│ 📄 pdf                              第 1 页    87 分   .pdf  ★│
│ 书评广告 新工科背景下材料实验课思政教学模式 评材料力学实验教程  │
│ ...随着新一轮科技变革的推进社会需要更多工程科技类人才...        │
│ ...为了适应【教育】改革加强培养学生的动手动脑能力...            │
└──────────────────────────────────────────────────────────────┘
```

- **位置标签**: "第 1 页" — 表示匹配内容出现在文档第1页
- **相关度分数**: "87 分" — 绿色显示,表示高度相关（≥80分为绿色）
- **文件类型标签**: ".pdf" — 标识文件扩展名
- **内容预览**: 搜索关键词"教育"以高亮颜色标注,方便定位匹配位置

**视频搜索结果示例:**

```
┌──────────────────────────────────────────────────────────────┐
│ 🎥 教学录像第一章              00:01:30 - 00:03:45   视频    ★│
│ 讲解课程导论部分,介绍本学期的学习目标和课程大纲...              │
└──────────────────────────────────────────────────────────────┘
```

- **位置标签**: "00:01:30 - 00:03:45" — 紫色标签,表示匹配内容出现在视频的该时间段
- **文件类型图标**: 视频摄像机图标
- **文件类型标签**: "视频"
- **点击后**: 弹出视频播放器窗口,自动跳转到 00:01:30 时间点
