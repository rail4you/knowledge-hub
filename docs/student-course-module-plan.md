...
## 七、Phase 3 - 后续可优化项（已记录但暂不开发）

### 7.1 用户体验
- [ ] 课程学习页面加载状态、错误状态、空状态美化
- [ ] 习题答题计时器（记录实际用时、显示在提交记录中）
- [ ] 章节完成后弹出庆祝动效 / 下一章推荐
- [ ] 资源内容支持 Markdown 渲染（当前是纯文本 pre 展示）
- [ ] 移动端适配优化（资源详情/习题答题的移动端布局）

### 7.2 数据展示
- [ ] 学习曲线使用真实数据（当前 demo 数据）需要后端提供 daily_time_labels/values
- [ ] 知识掌握度雷达图（已有 `getKnowledgeMastery`，可在学习页面或我的学习页加雷达图）
- [ ] 学习时长热力图（按日期格子）
- [ ] 课程排行榜（同课同班同期）需要新增 `getClassRank` 接口
- [ ] 知识点掌握度报告（按难度分组统计）

### 7.3 后端补充
- [ ] `GET /api/app/learning/daily-time` - 返回最近 N 天每日学习时长（用于热力图/曲线）
- [ ] `GET /api/app/course/{id}/ranking` - 同课学生排行
- [ ] `GET /api/app/exercise/knowledge-mastery-summary` - 按知识点汇总的掌握度（不是列表）
- [ ] 错题本（错题自动加入错题本，可重新练习）

### 7.4 知识点掌握度
- [ ] 详情页的「学习进度」tab 改用掌握度雷达图而非条形列表
- [ ] 「我的学习」页加「学科」按鱼骨图展示
