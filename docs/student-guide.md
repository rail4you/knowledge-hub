...
- **相关度分数**: 如"87 分" — 绿色显示,表示高度相关（≥80分为绿色）
- **文件类型标签**: 如".pdf" — 标识文件扩展名
- **内容预览**: 搜索关键词以高亮颜色标注,方便定位匹配位置

**视频搜索结果示例:**

每条视频结果同样以卡片形式展示,但有以下区别:
- **位置标签**: 如"00:01:30 - 00:03:45" — 紫色标签,表示匹配内容出现在视频的该时间段
- **文件类型图标**: 视频摄像机图标
- **文件类型标签**: "视频"
- **点击后**: 弹出视频播放器窗口,自动跳转到匹配时间点
